...
impl<'a> SliceDecoder<'a> {
    ...
    pub(super) fn transform_tree(
        ...
        let split = if log2 <= max_tb
            && log2 > min_tb
            && depth < self.max_trafo_depth
        ...
        if self.cu_intra || depth != 0 || cbf_cb || cbf_cr {
            cbf_luma = self
                .cab
                .decode(CTX_CBF_LUMA + if depth == 0 { 1 } else { 0 })
                == 1;
        }
        self.transform_unit(
            x0, y0, xbase, ybase, log2, blk_idx, cbf_luma, cbf_cb, cbf_cr,
        )
    }
    ...
    fn transform_unit(
        ...
        if (cbf_luma || cbf_chroma) && self.pps.cu_qp_delta_enabled && !self.is_cu_qp_delta_coded {
            ...
            while v < 5
                && self
                    .cab
                    .decode(CTX_CU_QP_DELTA + if v == 0 { 0 } else { 1 })
                    == 1
            {
                v += 1;
            }
    ...
    fn intra_predict(
        ...
        let avail = |st: &PicState, xn: i32, yn: i32| -> bool {
            st.avail_n(&ac, xn, yn)
                && (!constrained || st.pred_mode[st.idx4(xn as usize, yn as usize)] == PRED_INTRA)
        };
        ...
        if accel::census::ALWAYS {
            accel::census::route(
                all_avail,
                &accel::census::RT_INTRA_ALL_AVAIL,
                &accel::census::RT_INTRA_SUBSTITUTED,
            );
        }
    ...
    fn residual_block(
        ...
        if self.pps.transform_skip_enabled
            && !self.cu_transquant_bypass
            && log2 <= self.pps.log2_max_transform_skip_block_size as usize
        {
            ...
            if coded {
                ...
                while nn_ >= 0 {
                    ...
                    let s = if np > 0 || !infer_sb_dc {
                        ...
                        let sig_ctx = if dc_sb && np == 0 {
                            0
                        } else {
                            sig_row[np & 15] as usize + sig_off
                        };
    ...
    fn reconstruct_residual(
        ...
        if kind != TransformKind::Bypass {
            ...
            if accel::census::ALWAYS {
                accel::census::route(
                    m.is_some(),
                    &accel::census::RT_TX_SCALED,
                    &accel::census::RT_TX_FLAT,
                );
            }
            itx::dequant(
                &mut self.coeffs[..nn],
        ...
        if accel::census::ALWAYS {
            ...
            cx::bump(
                &cx::RT_TX_NZ_AREA,
                (nz_w.clamp(1, n) * nz_h.clamp(1, n)) as u64,
            );
            cx::bump(&cx::RT_TX_FULL_AREA, (n * n) as u64);
        }
        itx::inverse_transform(
            &mut self.coeffs[..nn],
            &mut self.itx_tmp,
            n,
            nz_w,
        ...
        match dc {
            ...
            Some(v) => accel::pixel::add_residual_const(
                &mut plane.data[yb * stride + xb..],
                stride,
                v,
                coeffs,
                n,
