...
mod silent_zero_guards {
    ...
    fn every_census_counter_is_actually_incremented() {
        ...
        assert!(
            files.len() > 5,
            "source walk found nothing; the guard would pass vacuously"
        );

        let lib = files
            .iter()
            .find(|p| {
                p.ends_with("rusty_h265-accel/src/lib.rs")
                    || p.to_string_lossy()
                        .replace('\\', "/")
                        .ends_with("rusty_h265-accel/src/lib.rs")
            })
            .expect("accel lib.rs");
        ...
        assert!(
            names.len() > 20,
            "expected the full census, found {}",
            names.len()
        );
        ...
        let dead: Vec<&String> = names
            .iter()
            .filter(|n| !bodies.contains(n.as_str()))
            .collect();
        assert!(
            dead.is_empty(),
