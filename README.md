...
| YouTube | Complete native H.264/AAC playback verified for one public short clip. The longer test requires client verification and fails explicitly; general YouTube support is incomplete |
...
- H.264 in ordinary MP4/MOV containers, plus assembled YouTube SABR video fragments. WebM, VP9, AV1, HEVC, HLS, DASH manifests, DRM, subtitles and live streams are not implemented.
...
- YouTube loads the complete clip before playback, with limits of 128 MiB received media and 20 minutes of declared duration. These limits do not imply that videos of that length are accessible. A tested longer video stopped at client verification after about a minute of media. No verification-token generator or browser fallback is included. See [YouTube support and limits](docs/youtube.md).
...
On the development machine, the decoder probe processed 60 generated 1280×720 H.264 frames in 0.786 seconds, including opening the file and lookahead. This is a single local probe, not a general performance benchmark. FixupX returned 60 decodable 482×360 frames with a 19.034-second duration. The updated YouTube adapter decoded all 284 frames and the complete audio of the short test clip through SABR. The final Windows release probe loaded that clip and decoded 60 frames in 4.416 seconds. Native controls passed with both renderers. The longer test returned an explicit client-verification error; it was not presented as a complete playable clip.
...
The next steps are a compatible client-verification integration for broader YouTube access, progressive segment playback, additional Rust codecs, and hardware decoding through native platform APIs. Rendering general web pages would be a separate project.
