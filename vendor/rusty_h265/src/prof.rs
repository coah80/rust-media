...
pub fn report(total_ns: u64) -> String {
    ...
    let nested: u64 = NESTED_IN_PARSE
        .iter()
        .map(|s| NS[*s as usize].load(Ordering::Relaxed))
        .sum();
    let in_inter: u64 = NESTED_IN_INTER
        .iter()
        .map(|s| NS[*s as usize].load(Ordering::Relaxed))
        .sum();
    let in_mc: u64 = NESTED_IN_MC
        .iter()
        .map(|s| NS[*s as usize].load(Ordering::Relaxed))
        .sum();
    let in_sao: u64 = NESTED_IN_SAO
        .iter()
        .map(|s| NS[*s as usize].load(Ordering::Relaxed))
        .sum();
    let in_dpb: u64 = NESTED_IN_DPB
        .iter()
        .map(|s| NS[*s as usize].load(Ordering::Relaxed))
        .sum();
    for i in 0..N {
    ...
    out.push_str(&format!(
        ...
        residue as f64 / 1e6,
        if total_ns > 0 {
            100.0 * residue as f64 / total_ns as f64
        } else {
            0.0
        }
    ));
