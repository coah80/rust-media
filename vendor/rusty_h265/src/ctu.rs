...
pub(crate) fn wrap_qp(v: i32, qp_bd_offset: i32) -> i32 {
    ...
    debug_assert!(
        t > -m && t < 2 * m,
        "wrap_qp dividend {t} outside -m..2m (m = {m})"
    );
    while t >= m {
        t -= m;
    }
...
impl<'a> SliceDecoder<'a> {
    pub fn new(inp: SliceInputs<'a>) -> Result<Self> {
        ...
        let cab = Cabac::new(
            &rbsp.data,
            sh.data_offset,
            Contexts::init(init_type, sh.slice_qp),
        );
        let ds_qp = inp.store.ds_qp;
        let refs = inp.refs;
        let no_backward_pred = refs
            .l0
            .iter()
            .chain(refs.l1.iter())
            .all(|r| r.poc <= inp.poc);
        let col = if sh.temporal_mvp_enabled && !sh.slice_type.is_intra() {
            let list = if sh.slice_type.is_b() && !sh.collocated_from_l0 {
                &refs.l1
            } else {
                &refs.l0
            };
        ...
        Ok(SliceDecoder {
            ...
            last_cu_qp: if sh.dependent_slice_segment {
                ds_qp.unwrap_or(sh.slice_qp)
            } else {
                sh.slice_qp
            },
    ...
    fn row_start(&self, rs: usize, ts: usize) -> bool {
        self.ctb_div.rem(rs) == 0
            || (ts > 0
                && self.tiles.tile_id[ts]
                    != self.tiles.tile_id[self.tiles.rs_to_ts[rs - 1] as usize])
    }
    ...
    pub fn decode(mut self) -> Result<()> {
        ...
        if self.sh.dependent_slice_segment
            && !self.first_in_tile(ts)
            && (self.tile_width_ctus(rs) >= 2 || !wpp)
        {
        ...
        loop {
            ...
            if self.first_in_tile(ts) {
                ...
                if self
                    .st
                    .available(x0 as i32, y0 as i32, x0 as i32 + ctb, y0 as i32 - ctb)
                {
            ...
            if wpp
                && (self.ctb_div.rem(rs) == 1
                    || (rs > 1
                        && self.tiles.tile_id[ts]
                            != self.tiles.tile_id[self.tiles.rs_to_ts[rs - 2] as usize]))
            {
    ...
    fn coding_quadtree(&mut self, x0: usize, y0: usize, log2cb: usize, depth: u8) -> Result<()> {
        ...
        let split = if x0 + size <= w && y0 + size <= h && log2cb > min_cb {
            ...
            self.cab
                .decode(CTX_SPLIT_CU + cond_l as usize + cond_a as usize)
                == 1
        } else {
            log2cb > min_cb
        };
        ...
        let log2_min_qg =
            self.sps.log2_ctb_size as usize - self.pps.diff_cu_qp_delta_depth as usize;
        if log2cb == log2_min_qg || (log2cb > log2_min_qg && !split) {
            ...
            let qp_prev = if self.qp_prev_reset {
                self.sh.slice_qp
            } else {
                self.last_cu_qp
            };
    ...
    fn coding_unit(&mut self, x0: usize, y0: usize, log2cb: usize, depth: u8) -> Result<()> {
        ...
        self.cu_transquant_bypass =
            self.pps.transquant_bypass_enabled && self.cab.decode(CTX_TRANSQUANT_BYPASS) == 1;
        let xi = x0 as i32;
        let yi = y0 as i32;
        let mut skip = false;
        if !self.sh.slice_type.is_intra() {
            ...
            skip = self
                .cab
                .decode(CTX_CU_SKIP + cond_l as usize + cond_a as usize)
                == 1;
        }
        self.qp_y = wrap_qp(
            self.qp_y_pred + self.cu_qp_delta_val,
            self.sps.qp_bd_offset_y,
        );
        PicState::fill4(&mut self.st.ct_depth, w4, x0, y0, n, n, depth);
        self.set_cu_qp();
        PicState::fill4(
            &mut self.st.filter_bypass,
            w4,
            x0,
            y0,
            n,
            n,
            self.cu_transquant_bypass as u8,
        );
        ...
        self.cu_intra = if self.sh.slice_type.is_intra() {
            true
        } else {
            self.cab.decode(CTX_PRED_MODE) == 1
        };
        let min_cb = self.sps.log2_min_cb_size as usize;
        self.part_mode = PartMode::Part2Nx2N;
    ...
    fn dbg_cu(&self, x0: usize, y0: usize, n: usize) {
        ...
        if self.cu_intra {
            eprintln!(
                "cu poc={} x={x0} y={y0} w={n} intra qp={} part={part} mode={}",
                self.poc, self.qp_y, self.st.intra_mode[i]
            );
        } else {
    ...
    fn parse_part_mode(&mut self, log2cb: usize) -> Result<PartMode> {
        ...
        if !self.sps.amp_enabled {
            return Ok(if self.cab.decode(CTX_PART_MODE + 1) == 1 {
                PartMode::Part2NxN
            } else {
                PartMode::PartNx2N
            });
        ...
        if self.cab.decode(CTX_PART_MODE + 1) == 1 {
            ...
            return Ok(if self.cab.bypass() == 0 {
                PartMode::Part2NxnU
            } else {
                PartMode::Part2NxnD
            });
    ...
    fn parse_intra_modes(&mut self, x0: usize, y0: usize, n: usize) -> Result<()> {
        let parts = if self.part_mode == PartMode::PartNxN {
            4
        } else {
            1
        };
        ...
        let icpm = if self.cab.decode(CTX_INTRA_CHROMA_PRED_MODE) == 0 {
            4
        } else {
            self.cab.bypass_bits(2) as u8
        };
