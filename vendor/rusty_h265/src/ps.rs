...
fn parse_sub_layer_ordering(
    r: &mut BitReader,
    max_sub_layers_minus1: u32,
) -> Result<[SubLayerOrdering; 8]> {
    ...
    fn default() -> Self {
        ...
        for size_id in 1..4 {
            for matrix_id in 0..6 {
                lists[size_id][matrix_id] = if matrix_id < 3 {
...
pub fn parse_st_ref_pic_set(
    ...
    if inter_rps_pred {
        let delta_idx_minus1 = if in_slice_header {
            r.read_ue_max(idx as u32 - 1)? as usize
        } else {
            0
        };
        ...
        let rf = sets
            .get(ref_idx)
            .ok_or_else(|| Error::invalid("st_ref_pic_set ref index"))?;
...
impl Sps {
    ...
    pub fn output_size(&self) -> (u32, u32) {
        ...
        let w = self
            .width
            .saturating_sub(sw * (self.conf_win[0] + self.conf_win[1]));
        let h = self
            .height
            .saturating_sub(sh * (self.conf_win[2] + self.conf_win[3]));
        (w, h)
    }
}
...
pub fn parse_sps(rbsp: &[u8]) -> Result<Sps> {
    ...
    if s.log2_max_tb_size > 5
        || s.log2_max_tb_size > s.log2_ctb_size
        || s.log2_min_tb_size >= s.log2_min_cb_size
    {
    ...
    if s.scaling_list_enabled {
        s.scaling_list = Some(if r.read_flag()? {
    ...
    if s.pcm_enabled {
        ...
        if s.pcm_bit_depth_luma > s.bit_depth_luma
            || s.pcm_bit_depth_chroma > s.bit_depth_chroma
            || s.log2_max_pcm_cb_size > 5.min(s.log2_ctb_size)
        {
    ...
    // derived
    s.chroma_array_type = if s.separate_colour_plane_flag {
        0
    } else {
        s.chroma_format_idc
    };
    ...
    if sw * (s.conf_win[0] + s.conf_win[1]) >= s.width
        || sh * (s.conf_win[2] + s.conf_win[3]) >= s.height
    {
...
impl TileLayout {
    pub fn new(sps: &Sps, pps: &Pps) -> Result<TileLayout> {
        ...
        for rs in 0..n as u32 {
            ...
            ts += (tb_y - row_bd[tile_y]) * (col_bd[tile_x + 1] - col_bd[tile_x]) + tb_x
                - col_bd[tile_x];
...
mod tests {
    ...
    fn default_scaling_tables_have_64_entries_and_are_monotone_ish() {
        ...
        assert_eq!(
            DEFAULT_SCALING_INTRA.iter().map(|&v| v as u32).sum::<u32>(),
            1939
        );
        assert_eq!(
            DEFAULT_SCALING_INTER.iter().map(|&v| v as u32).sum::<u32>(),
            1752
        );
    }
