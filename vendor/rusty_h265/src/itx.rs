...
mod tests {
            ...
            for k in 0..n {
                for j in 0..n {
                    ...
                    if k % 2 == 0 {
                        assert_eq!(
                            a, b,
                            "even row {k} of the {n}-point matrix is not symmetric at {j}"
                        );
                    } else {
                        assert_eq!(
                            a, -b,
                            "odd row {k} of the {n}-point matrix is not antisymmetric at {j}"
                        );
                    }
                }
            }
    ...
    fn transform_accumulator_fits_i32() {
        let worst_col = (0..32)
            .map(|j| (0..32).map(|k| (DCT32[k][j] as i64).abs()).sum::<i64>())
            .max()
            .unwrap();
        let worst = COEFF_MAX.max(-COEFF_MIN) as i64 * worst_col;
        assert!(
            worst <= i32::MAX as i64,
            "accumulator needs i64: worst case {worst} against {}",
            i32::MAX
        );
        ...
        assert!(
            headroom >= 4,
            "only {headroom}x headroom left; re-derive before narrowing further"
        );
