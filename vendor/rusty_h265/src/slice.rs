...
fn parse_pred_weight_table(
    ...
    let offset_shift_l = if high_precision {
        0
    } else {
        sps.bit_depth_luma as i32 - 8
    };
    let offset_shift_c = if high_precision {
        0
    } else {
        sps.bit_depth_chroma as i32 - 8
    };
    let wp_offset_half_c = 1
        << (if high_precision {
            sps.bit_depth_chroma - 1
        } else {
            7
        });
...
pub fn parse_slice_header<'a>(
    ...
    let (sps, pps) = sps_by_pps(pps_id)
        .ok_or_else(|| Error::invalid(format!("slice refers to unknown PPS {pps_id}")))?;
    ...
    let mut sh = if dependent_slice_segment {
        let p = prev
            .ok_or_else(|| Error::invalid("dependent slice segment without an independent one"))?;
        let mut sh = p.clone();
    ...
    if !dependent_slice_segment {
        ...
        if !nal.nal_type.is_idr() {
            ...
            if !sh.short_term_ref_pic_set_sps_flag {
                ...
                let idx = if sps.st_rps.len() > 1 {
                    r.read_bits(ceil_log2(sps.st_rps.len() as u32))? as usize
                } else {
                    0
                };
                sh.st_rps = sps
                    .st_rps
                    .get(idx)
                    .ok_or_else(|| Error::invalid("short_term_ref_pic_set_idx"))?
                    .clone();
            }
        ...
        if !sh.slice_type.is_intra() {
            ...
            sh.num_ref_idx_l1_active = if sh.slice_type.is_b() {
            ...
            if sh.temporal_mvp_enabled {
                ...
                if (sh.collocated_from_l0 && sh.num_ref_idx_l0_active > 1)
                    || (!sh.collocated_from_l0 && sh.num_ref_idx_l1_active > 1)
                {
                    sh.collocated_ref_idx = r.read_ue_max(14)? as u8;
                    let n = if sh.collocated_from_l0 {
                        sh.num_ref_idx_l0_active
                    } else {
                        sh.num_ref_idx_l1_active
                    };
            ...
            if (pps.weighted_pred && sh.slice_type == SliceType::P)
                || (pps.weighted_bipred && sh.slice_type.is_b())
            {
        ...
        if deblocking_override {
            ...
            if !sh.deblocking_filter_disabled {
                ...
                if !(-6..=6).contains(&sh.beta_offset_div2)
                    || !(-6..=6).contains(&sh.tc_offset_div2)
                {
        ...
        if num > 0 {
            ...
            for _ in 0..num {
                sh.entry_point_offsets
                    .push(r.read_bits(len)?.wrapping_add(1));
            }
        }
    }
