...
impl RefSamples {
    ...
    pub fn substitute(&mut self, n: usize, bit_depth: u8, all: bool) {
        ...
        let n2 = 2 * n;
        let any = self.corner_avail
            || self.left_avail[..n2].iter().any(|&a| a)
            || self.top_avail[..n2].iter().any(|&a| a);
        if !any {
    ...
    fn filter(&mut self, n: usize, mode: u8, bit_depth: u8, strong_intra_smoothing: bool) {
        ...
        let bi_int = strong_intra_smoothing
            && n == 32
            && (corner + self.top[n2 - 1] as i32 - 2 * self.top[n - 1] as i32).abs()
                < (1 << (bit_depth - 5))
            && (corner + self.left[n2 - 1] as i32 - 2 * self.left[n - 1] as i32).abs()
                < (1 << (bit_depth - 5));
        if bi_int {
...
pub fn predict(
    ...
    if c_idx == 0 {
        ...
        if accel::census::ALWAYS {
            accel::census::route(
                refs.corner != before,
                &accel::census::RT_INTRA_REF_FILTERED,
                &accel::census::RT_INTRA_REF_PLAIN,
            );
        }
    }
    ...
    match mode {
        ...
        MODE_DC => {
            ...
            if accel::census::ALWAYS {
                accel::census::route(
                    defer,
                    &accel::census::RT_INTRA_DC_DEFERRED,
                    &accel::census::RT_INTRA_DC_FILLED,
                );
            }
            if defer {
        ...
        _ => {
            ...
            if accel::census::ALWAYS {
                accel::census::route(
                    mode >= 18,
                    &accel::census::RT_INTRA_ANG_ROW,
                    &accel::census::RT_INTRA_ANG_TRANSPOSED,
                );
            }
