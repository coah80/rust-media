...
impl PuMv {
    fn from_motion(m: &Motion) -> Self {
        PuMv {
            mv: [
                [m.mv[0][0] as i32, m.mv[0][1] as i32],
                [m.mv[1][0] as i32, m.mv[1][1] as i32],
            ],
    ...
    fn same(&self, o: &PuMv) -> bool {
        ...
        for l in 0..2 {
            if self.flags & (1 << l) != 0
                && (self.mv[l] != o.mv[l] || self.ref_idx[l] != o.ref_idx[l])
            {
                return false;
            }
        }
...
impl<'a> SliceDecoder<'a> {
    ...
    pub(super) fn inter_prediction_units(
        &mut self,
        x0: usize,
        y0: usize,
        n: usize,
    ...
    pub(super) fn prediction_unit(
        ...
        let pu = if merge {
            ...
            let ref_idx = |s: &mut Self, num: u8| -> i32 {
                ...
                while i < cmax {
                    let bin = if i < 2 {
                        s.cab.decode(CTX_REF_IDX + i as usize)
                    } else {
                        s.cab.bypass()
                    };
                    if bin == 0 {
    ...
    fn merge_motion(
        ...
        let (xp, yp, w, h, part_idx) = if plevel > 2 && ncb == 8 {
            (xcb, ycb, ncb, ncb, 0)
        } else {
            (xp0, yp0, w0, h0, part_idx0)
        };
        let pm = self.part_mode;
        let same_mer = |xn: i32, yn: i32| -> bool {
            (xp >> plevel) as i32 == xn >> plevel && (yp >> plevel) as i32 == yn >> plevel
        };
        ...
        let vert_second = part_idx == 1
            && matches!(
                pm,
                PartMode::PartNx2N | PartMode::PartnLx2N | PartMode::PartnRx2N
            );
        let a1 = if !same_mer(xa1, ya1) && !vert_second {
            self.pb_avail(&ac, xcb, ycb, ncb, w, h, part_idx, xa1, ya1)
                .map(|i| self.motion_at_idx(i))
        } else {
            None
        };
        if let Some(m) = a1 {
        ...
        let b0_i = if same_mer(xb0, yb0) {
            None
        } else {
            self.pb_avail(&ac, xcb, ycb, ncb, w, h, part_idx, xb0, yb0)
        };
        if let Some(i) = b0_i {
        ...
        let a0_i = if same_mer(xa0, ya0) {
            None
        } else {
            self.pb_avail(&ac, xcb, ycb, ncb, w, h, part_idx, xa0, ya0)
        };
        if let Some(i) = a0_i {
            let m = self.motion_at_idx(i);
            if !a1.is_some_and(|a| a.same(&m)) {
        ...
        if cands.len() != 4 {
            ...
            let b2_i = if same_mer(xb2, yb2) {
                None
            } else {
                self.pb_avail(&ac, xcb, ycb, ncb, w, h, part_idx, xb2, yb2)
            };
            if let Some(i) = b2_i {
                let m = self.motion_at_idx(i);
        ...
        let mut m = *cands
            .get(merge_idx)
            .ok_or_else(|| Error::invalid("merge_idx beyond the candidate list"))?;
    ...
    fn col_mv(
        ...
        let target = if lx == 0 {
            &self.refs.l0[ref_idx as usize]
        } else {
            &self.refs.l1[ref_idx as usize]
        };
    ...
    fn amvp(
        ...
        let avail = |p: (i32, i32)| {
            self.pb_avail(&ac, xcb, ycb, ncb, w, h, part_idx, p.0, p.1)
                .map(|i| self.motion_at_idx(i))
        };
    ...
    fn store_motion(&mut self, xp: usize, yp: usize, w: usize, h: usize, pu: &PuMv) {
        ...
        for l in 0..2 {
            if pu.flags & (1 << l) != 0 {
                ...
                let r = if l == 0 {
                    &self.refs.l0[pu.ref_idx[l] as usize]
                } else {
                    &self.refs.l1[pu.ref_idx[l] as usize]
                };
                m.ref_poc[l] = r.poc;
                if r.is_long_term {
                    m.ref_lt |= 1 << l;
    ...
    fn motion_compensate(
        ...
        for c in 0..3usize {
            ...
            if weights.is_none() && bit_depth <= 12 {
                for l in 0..2usize {
                    ...
                    if fx == 0
                        ...
                        && xi >= 0
                        && yi >= 0
                        && xi as usize + bw <= p.width
                        && yi as usize + bh <= p.height
                    {
            ...
            for l in 0..2usize {
                ...
                let interior = x0 >= 0
                    && y0 >= 0
                    && (x0 as usize + fw) <= plane.width
                    && (y0 as usize + fh) <= plane.height;
                let (src, stride): (&[u16], usize) = if interior {
                    (
                        &plane.data[y0 as usize * plane.stride + x0 as usize..],
                        plane.stride,
                    )
                } else {
            ...
            match (used[0], used[1], weights) {
                (true, false, None) | (false, true, None) => {
                    ...
                    match fp[l] {
                        Some((o, ss2)) => {
                            accel::pixel::copy_block(dst, stride, &refp(l)[o..], ss2, bw, bh)
                        }
                        None => {
                            accel::pixel::put_uni(dst, stride, &scratch.pred[l], bw, bh, bit_depth)
                        }
                    }
                }
                (true, true, None) => match (fp[0], fp[1]) {
                    (Some((o0, s0)), Some((o1, s1))) => accel::pixel::avg_block(
                        dst,
                        stride,
                        &refp(0)[o0..],
                        s0,
                    ...
                    (Some((o0, s0)), None) => accel::pixel::put_bi_fp(
                        dst,
                        stride,
                        &refp(0)[o0..],
                        s0,
                    ...
                    (None, Some((o1, s1))) => accel::pixel::put_bi_fp(
                        dst,
                        stride,
                        &refp(1)[o1..],
                        s1,
