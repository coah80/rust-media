...
use crate::ps::{
    parse_pps, parse_sps, parse_vps, Pps, Sps, TileLayout, Vps, PROFILE_MAIN, PROFILE_MAIN10,
    PROFILE_MAIN_STILL,
};
...
impl Decoder {
    ...
    fn push_slice(&mut self, hdr: &NalHeader, rbsp: Rbsp) -> Result<()> {
        ...
        let cur = self
            .cur
            .as_mut()
            .ok_or_else(|| Error::invalid("no current picture"))?;
        ...
        let lists = if sh.slice_type.is_intra() {
            ...
            build_ref_lists(&self.dpb, &sh, cur)?
        };
    ...
    fn start_picture(&mut self, hdr: &NalHeader, sh: &SliceHeader) -> Result<()> {
        let pps = self.pps[sh.pps_id as usize]
            .clone()
            .ok_or_else(|| Error::invalid("PPS"))?;
        let sps = self.sps[pps.sps_id as usize]
            .clone()
            .ok_or_else(|| Error::invalid("SPS"))?;
        let nt = hdr.nal_type;
        ...
        check_scope(&sps, &pps)?;
        let sps_changed = self.active_sps_id != Some(sps.id)
            || self.dpb.iter().any(|e| {
                e.pic.width() != sps.width as usize || e.pic.height() != sps.height as usize
            });
        ...
        if nt.is_idr() {
                ...
                .chain(st_foll.iter())
            {
                if let Some(i) = self
                    .dpb
                    .iter()
                    .position(|e| e.mark == RefMark::ShortTerm && e.poc == *p)
                {
            ...
            for p in st_before.iter().chain(st_after.iter()) {
                if !self
                    .dpb
                    .iter()
                    .any(|e| e.mark == RefMark::ShortTerm && e.poc == *p)
                {
            ...
            loop {
                ...
                let latency_hit = ord.max_latency_increase_plus1 != 0
                    && self.dpb.iter().any(|e| {
                        e.needed_for_output
                            && e.pic_latency
                                >= ord.max_num_reorder_pics + ord.max_latency_increase_plus1 - 1
                    });
        ...
        let output_flag = if nt.is_rasl() && self.no_rasl_output_flag {
            false
        } else {
            sh.pic_output_flag
        };
        ...
        if !self
            .seq_tables
            .as_ref()
            .is_some_and(|t| t.matches(&sps, &tiles))
        {
...
fn check_scope(sps: &Sps, pps: &Pps) -> Result<()> {
    ...
    if !matches!(prof, 0 | PROFILE_MAIN | PROFILE_MAIN10 | PROFILE_MAIN_STILL) {
        return Err(Error::unsupported(format!(
            "general_profile_idc {prof} (only Main / Main 10 / Main Still Picture)"
        )));
    }
    ...
    let pick = |temp: &[RefPic], modif: &Option<Vec<u32>>, n: usize| -> Result<Vec<RefPic>> {
        (0..n)
            .map(|i| {
                ...
                temp.get(idx)
                    .cloned()
                    .ok_or_else(|| Error::invalid("list_entry out of range"))
            })
            .collect()
    };
    let l0 = pick(
