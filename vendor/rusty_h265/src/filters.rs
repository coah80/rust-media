...
pub fn apply_in_loop_filters(
    cur: &mut CurrentPicture,
    scratch: &mut crate::decoder::FilterScratch,
) {
    ...
    let any_sao = sps.sao_enabled
        && cur
            .state
            .ctb_filter
            .iter()
            .any(|f| f.sao_luma || f.sao_chroma);
...
fn boundary_strength(st: &PicState, p: usize, q: usize, tu_edge: bool) -> u8 {
    ...
    if np == 1 {
        let (rp, vp) = if mp.pred_flags & 1 != 0 {
            (mp.ref_poc[0], mp.mv[0])
        } else {
            (mp.ref_poc[1], mp.mv[1])
        };
        let (rq, vq) = if mq.pred_flags & 1 != 0 {
            (mq.ref_poc[0], mq.mv[0])
        } else {
            (mq.ref_poc[1], mq.mv[1])
        };
        if rp != rq {
            return 1;
        }
    ...
    for c in 0..3usize {
        ...
        for ry in 0..st.ctb_h {
            for rx in 0..st.ctb_w {
                ...
                if accel::census::ALWAYS {
                    accel::census::route(
                        interior_ok && use_kernel,
                        &accel::census::RT_SAO_INTERIOR,
                        &accel::census::RT_SAO_RING,
                    );
                }
